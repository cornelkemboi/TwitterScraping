...
- Twitter is experiencing unusual activity"""
    ...
    def save_to_mysql(self):
        print("Saving Tweets to MySQL database...")
        try:
                    ...
                    for tweet in self.data:
                        ...
                        for item in tweet:
                            if isinstance(item, list):
                                processed_tweet.append(str(item[0]) if item else None)
                            else:
                                processed_tweet.append(item)

                        # Convert timestamp to MySQL DATETIME format
                        iso_timestamp = processed_tweet[2]
                        mysql_timestamp = datetime.strptime(iso_timestamp, '%Y-%m-%dT%H:%M:%S.%fZ').strftime(
                            '%Y-%m-%d %H:%M:%S')

                        data = (
                            str(processed_tweet[14]),  # tweet_id
                            processed_tweet[0],  # name
                            processed_tweet[1],  # handle
                            mysql_timestamp,  # timestamp (converted)
                            processed_tweet[3],  # verified
                            processed_tweet[4],  # content
                            processed_tweet[5],  # comments
                            processed_tweet[6],  # retweets
                            processed_tweet[7],  # likes
                            processed_tweet[8],  # analytics
                            processed_tweet[9],  # tags
                            processed_tweet[10],  # mentions
                            processed_tweet[11],  # emojis
                            processed_tweet[12],  # profile_image
                            processed_tweet[13],  # tweet_link
                            processed_tweet[15] if self.scraper_details["poster_details"] else None,  # tweeter_id
                            processed_tweet[16] if self.scraper_details["poster_details"] else None,  # following
                            processed_tweet[17] if self.scraper_details["poster_details"] else None  # followers
                        )

                        sql_insert = """
                            INSERT INTO tweets_fb (
                                tweet_id, name, handle, timestamp, verified, content, comments, retweets, likes,
                                analytics, tags, mentions, emojis, profile_image, tweet_link, tweeter_id, following, followers
